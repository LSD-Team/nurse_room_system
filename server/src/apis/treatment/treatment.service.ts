...
export class TreatmentService {
  ...
  async getPatientProfile(query: IPatientProfileQuery, createdBy: string) {
    ...
    const profileResult = await this.db.executeStoredProcedure<{
      patient_id: number;
      action: string;
    ...
    if (!patientId) {
      throw new InternalServerErrorException(
        'ไม่สามารถดึงข้อมูล patient profile ได้',
      );
    }

    // 2. Parallel queries
    const [profileData, allergies, diseases, visitCount, latestVitals] =
      await Promise.all([
        this.db.query<any>(
          db,
    ...
    const latestPhysical = lv
      ? {
          ...
          bmi:
            lv.weight_kg && lv.height_cm
              ? Math.round(
                  (lv.weight_kg / Math.pow(lv.height_cm / 100, 2)) * 10,
                ) / 10
              : null,
  ...
  async upsertDisease(body: IUpsertDiseaseBody, createdBy: string) {
    return this.db.executeStoredProcedure(
      this.DATABASE_NAME,
      'sp_PP_03_UpsertUnderlyingDisease',
      {
  ...
  async getLookups() {
    ...
    const [
      ...
      this.db.query<any>(
        db,
        `SELECT treatment_type_id AS id, treatment_code AS code, treatment_name_th AS name_th, treatment_name_en AS name_en, sort_order, is_active FROM treatment_types WHERE is_active = 1 ORDER BY sort_order`,
      ),
      this.db.query<any>(
        db,
        `SELECT refer_type_id AS id, refer_code AS code, refer_name_th AS name_th, refer_name_en AS name_en, sort_order, is_active FROM refer_types WHERE is_active = 1 ORDER BY sort_order`,
      ),
      this.db.query<any>(
        db,
        `SELECT severity_id AS id, severity_code AS code, severity_name_th AS name_th, severity_name_en AS name_en, sort_order, is_active FROM accident_severity_types WHERE is_active = 1 ORDER BY sort_order`,
      ),
      this.db.query<any>(
        db,
        `SELECT group_id AS id, group_code AS code, group_name_th AS name_th, group_name_en AS name_en, sort_order, is_active FROM disease_groups WHERE is_active = 1 ORDER BY sort_order`,
      ),
      this.db.query<any>(
        db,
        `SELECT sub_group_id AS id, group_id, sub_group_code AS code, sub_group_name_th AS name_th, sub_group_name_en AS name_en, sort_order, is_active FROM disease_sub_groups WHERE is_active = 1 ORDER BY group_id, sort_order`,
      ),
      this.db.query<any>(
        db,
        `SELECT hospital_id, hospital_code, hospital_name_th, hospital_name_en, hospital_type, is_active FROM hospitals WHERE is_active = 1 ORDER BY hospital_code`,
      ),
    ]);
  ...
  async getLastApprovedStockDate(): Promise<{
    last_approved_date: string | null;
  }> {
    const result = await this.db.executeStoredProcedure(
      this.DATABASE_NAME,
      'sp_TR_05_GetLastApprovedStockDate',
      {},
    );
    return {
      last_approved_date: (result[0] as any)?.last_approved_date ?? null,
    };
  }
  ...
  async updateVisit(visitId: number, body: IUpdateVisitBody, editedBy: string) {
    ...
    const result = await this.db.executeStoredProcedure(
      ...
      'sp_TR_06_UpdateVisit',
      {
        ...
        TreatmentTypeId: body.treatment_type_id ?? null,
        AccidentInWorkFlag:
          body.accident_in_work_flag != null
            ? body.accident_in_work_flag
              ? 1
              : 0
            : null,
        WorkRelatedFlag:
          body.work_related_flag != null
            ? body.work_related_flag
              ? 1
              : 0
            : null,
