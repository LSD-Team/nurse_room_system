...
const filteredHeaders = computed(() => {
  ...
  return numberedHeaders.value.filter((item) => item.borrow_status === selectedStatusFilter.value);
});
...
const selectedItemInfo = computed(() => {
  ...
  return supplierPrices.value.find((p) => p.item_id === selectedItemId.value) || null;
});
...
async function onSupplierChange() {
  ...
  try {
    supplierPrices.value = await BorrowService.getSupplierPrices(selectedSupplierId.value);
  } catch (err: any) {
    errorMsg.value = err?.response?.data?.message || err?.message || String(err);
  }
}
...
function addLine() {
  ...
  const price = supplierPrices.value.find((p) => p.item_id === selectedItemId.value);
  if (!price) return;
...
async function saveBorrow() {
  if (!selectedSupplierId.value || formLines.value.length === 0) {
    await Swal.fire('\u0E41\u0E08\u0E49\u0E07\u0E40\u0E15\u0E37\u0E2D\u0E19', '\u0E01\u0E23\u0E38\u0E13\u0E32\u0E40\u0E25\u0E37\u0E2D\u0E01 Supplier \u0E41\u0E25\u0E30\u0E40\u0E1E\u0E34\u0E48\u0E21\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23\u0E22\u0E32\u0E2D\u0E22\u0E48\u0E32\u0E07\u0E19\u0E49\u0E2D\u0E22 1 \u0E23\u0E32\u0E22\u0E01\u0E32\u0E23', 'warning');
    return;
  }
  ...
  try {
    if (isEditing.value && editingBorrowId.value) {
      ...
      await loadBorrowHeaders();
      await Swal.fire('\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08', '\u0E41\u0E01\u0E49\u0E44\u0E02\u0E43\u0E1A\u0E22\u0E37\u0E21\u0E40\u0E23\u0E35\u0E22\u0E1A\u0E23\u0E49\u0E2D\u0E22', 'success');
    } else {
      ...
      showFormDialog.value = false;
      await loadBorrowHeaders();
      await Swal.fire('\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08', '\u0E2A\u0E23\u0E49\u0E32\u0E07\u0E43\u0E1A\u0E22\u0E37\u0E21\u0E40\u0E23\u0E35\u0E22\u0E1A\u0E23\u0E49\u0E2D\u0E22', 'success');
    }
  } catch (err: any) {
...
async function viewDetail(row: IBorrowHeader) {
  ...
  try {
    const promises: Promise<any>[] = [BorrowService.getBorrowLines(row.borrow_id)];
...
async function receiveBorrow(row: IBorrowHeader) {
  const result = await Swal.fire({
    title: '\u0E22\u0E37\u0E19\u0E22\u0E31\u0E19\u0E23\u0E31\u0E1A\u0E22\u0E37\u0E21\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E15\u0E47\u0E2D\u0E01?',
    text: '\u0E43\u0E1A\u0E22\u0E37\u0E21 ' + row.borrow_no,
    icon: 'question',
    showCancelButton: true,
    confirmButtonText: '\u0E23\u0E31\u0E1A\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E15\u0E47\u0E2D\u0E01',
    cancelButtonText: '\u0E22\u0E01\u0E40\u0E25\u0E34\u0E01',
  });
  if (!result.isConfirmed) return;
  ...
  try {
    ...
    await Swal.fire('\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08', '\u0E23\u0E31\u0E1A\u0E22\u0E37\u0E21\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E15\u0E47\u0E2D\u0E01\u0E40\u0E23\u0E35\u0E22\u0E1A\u0E23\u0E49\u0E2D\u0E22', 'success');
    await loadBorrowHeaders();
  } catch (err: any) {
    // Error handled by axios interceptor
  }
...
async function cancelBorrow(row: IBorrowHeader) {
  ...
  try {
    await BorrowService.cancelBorrow(row.borrow_id, { Reason: value || undefined });
    await Swal.fire('\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08', '\u0E22\u0E01\u0E40\u0E25\u0E34\u0E01\u0E43\u0E1A\u0E22\u0E37\u0E21\u0E40\u0E23\u0E35\u0E22\u0E1A\u0E23\u0E49\u0E2D\u0E22', 'success');
    await loadBorrowHeaders();
  } catch (err: any) {
    // Error handled by axios interceptor
  }
...
<template>
  <div class="card">
    <div class="flex items-center justify-between mb-4">
      <div>
        <h2 class="text-2xl font-bold text-surface-900 dark:text-surface-0">{{"&#xe22;&#xe37;&#xe21;&#xe22;&#xe32;/&#xe40;&#xe27;&#xe0a;&#xe20;&#xe31;&#xe13;&#xe11;&#xe4c;"}}</h2>
        <p class="text-surface-500 mt-1">{{"&#xe23;&#xe32;&#xe22;&#xe01;&#xe32;&#xe23;&#xe43;&#xe1a;&#xe22;&#xe37;&#xe21;&#xe22;&#xe32;/&#xe40;&#xe27;&#xe0a;&#xe20;&#xe31;&#xe13;&#xe11;&#xe4c;&#xe17;&#xe31;&#xe49;&#xe07;&#xe2b;&#xe21;&#xe14;"}}</p>
      </div>
      <Button :label="'\u0E2A\u0E23\u0E49\u0E32\u0E07\u0E43\u0E1A\u0E22\u0E37\u0E21'" icon="pi pi-plus" @click="openCreateDialog" />
    </div>
    ...
    >
      ...
      <Column field="borrow_no" :header="'\u0E40\u0E25\u0E02\u0E17\u0E35\u0E48\u0E43\u0E1A\u0E22\u0E37\u0E21'" sortable style="min-width: 150px" frozen>
        <template #body="{ data }">
          <a class="text-primary cursor-pointer font-semibold hover:underline" @click="viewDetail(data)">
            {{ data.borrow_no }}
          </a>
        </template>
      </Column>
      <Column field="borrow_date" :header="'\u0E27\u0E31\u0E19\u0E17\u0E35\u0E48'" sortable style="min-width: 120px">
      ...
      <Column field="borrow_status" :header="'\u0E2A\u0E16\u0E32\u0E19\u0E30'" sortable style="min-width: 140px">
        <template #body="{ data }">
          <Tag :value="statusLabel(data.borrow_status)" :severity="statusSeverity(data.borrow_status)" />
        </template>
      </Column>
      <Column field="approval_role" :header="'\u0E23\u0E30\u0E14\u0E31\u0E1A\u0E2D\u0E19\u0E38\u0E21\u0E31\u0E15\u0E34'" style="min-width: 130px">
        <template #body="{ data }">
          {{ data.approval_role || '-' }}
        </template>
      </Column>
      <Column field="approval_status" :header="'\u0E2A\u0E16\u0E32\u0E19\u0E30\u0E2D\u0E19\u0E38\u0E21\u0E31\u0E15\u0E34'" style="min-width: 130px">
        <template #body="{ data }">
          {{ data.approval_status || '-' }}
        </template>
      </Column>
      <Column :header="'\u0E08\u0E31\u0E14\u0E01\u0E32\u0E23'" style="min-width: 200px" frozen alignFrozen="right">
        <template #body="{ data }">
          <div class="flex gap-1 flex-wrap">
            <template v-if="data.borrow_status === 'DRAFT'">
              <Button :label="'\u0E41\u0E01\u0E49\u0E44\u0E02'" icon="pi pi-pencil" severity="info" size="small" text @click="openEditDialog(data)" />
              <Button :label="'\u0E2A\u0E48\u0E07\u0E2D\u0E19\u0E38\u0E21\u0E31\u0E15\u0E34'" icon="pi pi-send" severity="success" size="small" text @click="submitBorrow(data)" />
              <Button :label="'\u0E22\u0E01\u0E40\u0E25\u0E34\u0E01'" icon="pi pi-times" severity="danger" size="small" text @click="cancelBorrow(data)" />
            </template>
            <template v-if="data.borrow_status === 'APPROVED'">
              <Button :label="'\u0E23\u0E31\u0E1A\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E15\u0E47\u0E2D\u0E01'" icon="pi pi-download" severity="success" size="small" text @click="receiveBorrow(data)" />
            </template>
    ...
    <Dialog
      ...
      :header="isEditing ? '\u0E41\u0E01\u0E49\u0E44\u0E02\u0E43\u0E1A\u0E22\u0E37\u0E21' : '\u0E2A\u0E23\u0E49\u0E32\u0E07\u0E43\u0E1A\u0E22\u0E37\u0E21\u0E43\u0E2B\u0E21\u0E48'"
      modal
      :style="{ width: '800px' }"
      :closable="true"
    >
      <div class="flex flex-col gap-4">
        ...
        <div v-if="selectedSupplierId" class="flex flex-col gap-2">
          <label class="font-semibold">{{"&#xe40;&#xe1e;&#xe34;&#xe48;&#xe21;&#xe23;&#xe32;&#xe22;&#xe01;&#xe32;&#xe23;&#xe22;&#xe32;"}}</label>
            ...
            >
              <template #value="{ placeholder }">
                <span v-if="selectedItemInfo">
                  {{ selectedItemInfo.item_name_th }} ({{ selectedItemInfo.item_name_en }}) - ฿{{ formatNumber(selectedItemInfo.unit_price) }}/{{ selectedItemInfo.unit_name_th }}
                </span>
                <span v-else class="text-surface-400">{{ placeholder }}</span>
              </template>
              <template #option="{ option }">
                <span>{{ option.item_name_th }} ({{ option.item_name_en }}) - ฿{{ formatNumber(option.unit_price) }}/{{ option.unit_name_th }}</span>
              </template>
            </Select>
            <div class="flex flex-col gap-1">
              <label class="text-sm font-semibold">{{"จำนวน"}}</label>
              <InputNumber v-model="selectedQty" :min="1" showButtons style="width: 140px" />
            </div>
            <Button icon="pi pi-plus" severity="success" @click="addLine" :disabled="!selectedItemId || selectedQty <= 0" class="mt-5" />
          </div>
        </div>

        <DataTable :value="formLines" dataKey="item_id" class="p-datatable-sm" v-if="formLines.length > 0">
          <Column field="item_name_th" :header="'\u0E0A\u0E37\u0E48\u0E2D\u0E22\u0E32'" style="min-width: 250px">
            <template #body="{ data }">
              <div>{{ data.item_name_th }}</div>
              <div class="text-sm text-surface-400">{{ data.item_name_en }}</div>
            </template>
          </Column>
          <Column field="qty" :header="'\u0E08\u0E33\u0E19\u0E27\u0E19'" style="min-width: 80px" bodyClass="text-right" />
          <Column field="unit_name_th" :header="'\u0E2B\u0E19\u0E48\u0E27\u0E22'" style="min-width: 100px" />
          <Column field="unit_price" :header="'\u0E23\u0E32\u0E04\u0E32/\u0E2B\u0E19\u0E48\u0E27\u0E22'" style="min-width: 110px" bodyClass="text-right">
            <template #body="{ data }">฿{{ formatNumber(data.unit_price ?? 0) }}</template>
          </Column>
          <Column field="total_price" :header="'\u0E23\u0E27\u0E21'" style="min-width: 110px" bodyClass="text-right">
            <template #body="{ data }">฿{{ formatNumber(data.total_price ?? 0) }}</template>
          </Column>
        ...
        <div v-if="formLines.length > 0" class="flex justify-end text-lg font-bold">
          {{"&#xe23;&#xe27;&#xe21;&#xe17;&#xe31;&#xe49;&#xe07;&#xe2b;&#xe21;&#xe14;: "}}฿{{ formatNumber(formTotalAmount) }}
        </div>
      </div>

      <template #footer>
        <Button :label="'\u0E22\u0E01\u0E40\u0E25\u0E34\u0E01'" icon="pi pi-times" severity="secondary" text @click="showFormDialog = false" />
        <Button :label="isEditing ? '\u0E1A\u0E31\u0E19\u0E17\u0E36\u0E01' : '\u0E2A\u0E23\u0E49\u0E32\u0E07\u0E43\u0E1A\u0E22\u0E37\u0E21'" icon="pi pi-check" @click="saveBorrow" />
      </template>
    </Dialog>
    ...
    >
      <div v-if="detailBorrow" class="flex flex-col gap-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <span class="font-semibold text-surface-500">{{"&#xe27;&#xe31;&#xe19;&#xe17;&#xe35;&#xe48;:"}}</span>
          ...
          <div>
            <span class="font-semibold text-surface-500">{{"&#xe2a;&#xe16;&#xe32;&#xe19;&#xe30;:"}}</span>
            <Tag class="ml-2" :value="statusLabel(detailBorrow.borrow_status)" :severity="statusSeverity(detailBorrow.borrow_status)" />
          </div>
          <div v-if="detailBorrow.approval_role">
            <span class="font-semibold text-surface-500">{{"&#xe23;&#xe30;&#xe14;&#xe31;&#xe1a;&#xe2d;&#xe19;&#xe38;&#xe21;&#xe31;&#xe15;&#xe34;:"}}</span>
            <span class="ml-2">{{ detailBorrow.approval_role }} (L{{ detailBorrow.approval_level }})</span>
          </div>
        ...
        <DataTable :value="detailLines" :loading="detailLoading" dataKey="borrow_line_id" class="p-datatable-sm">
          ...
          <Column field="qty_borrow" :header="'\u0E08\u0E33\u0E19\u0E27\u0E19'" style="min-width: 80px" bodyClass="text-right" />
          <Column field="purchase_unit_name_th" :header="'\u0E2B\u0E19\u0E48\u0E27\u0E22'" style="min-width: 100px" />
          <Column field="unit_price" :header="'\u0E23\u0E32\u0E04\u0E32/\u0E2B\u0E19\u0E48\u0E27\u0E22'" style="min-width: 110px" bodyClass="text-right">
            <template #body="{ data }">฿{{ formatNumber(data.unit_price) }}</template>
          </Column>
          <Column field="total_price" :header="'\u0E23\u0E27\u0E21'" style="min-width: 110px" bodyClass="text-right">
            <template #body="{ data }">฿{{ formatNumber(data.total_price) }}</template>
          </Column>
        </DataTable>

        <!-- Approval Timeline -->
        <div v-if="detailLogs.length > 0" class="mt-4">
          <div class="font-semibold text-surface-500 mb-2">Timeline &#x0E01;&#x0E32;&#x0E23;&#x0E2D;&#x0E19;&#x0E38;&#x0E21;&#x0E31;&#x0E15;&#x0E34;</div>
