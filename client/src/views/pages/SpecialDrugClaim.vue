...
const selectedStockItem = computed(() => {
  ...
  return stockItems.value.find((s) => s.item_id === selectedItemId.value) || null;
});
...
function addCreateLine() {
  ...
  if ((selectedStockItem.value.qty_base ?? 0) < qty) {
    void Swal.fire('Insufficient Stock', 'Withdrawal quantity exceeds available stock', 'warning');
    return;
  }
...
async function submitCreate() {
  if (!createReason.value?.trim()) {
    await Swal.fire('Incomplete Data', 'Please provide a reason for the withdrawal', 'warning');
    return;
  }
  if (createLines.value.length === 0) {
    await Swal.fire('Incomplete Data', 'Please add at least one item', 'warning');
    return;
  }
  ...
  try {
    ...
    await Swal.fire('Success', 'Special drug claim saved successfully', 'success');
...
async function submitEdit() {
  ...
  if (!editReason.value?.trim()) {
    await Swal.fire('Incomplete Data', 'Please provide a reason for the modification', 'warning');
    return;
  }
  ...
  for (const adj of adjustments) {
    const original = editItems.value.find((x) => x.claim_item_id === adj.claim_item_id);
    if (!original) continue;

    const delta = adj.new_qty_issued_base - original.qty_issued_base;
    if (delta > 0) {
      const stock = stockItems.value.find((s) => s.item_id === original.item_id);
  ...
  try {
    ...
    await Swal.fire('Success', 'Special drug claim modified successfully', 'success');
    showEditDialog.value = false;
...
async function submitReturn() {
  ...
  if (!returnReason.value?.trim()) {
    await Swal.fire('Incomplete Data', 'Please provide a clear reason for the return', 'warning');
    return;
  }

  const qty = Number(returnQty.value || 0);
  if (qty <= 0) {
    await Swal.fire('Invalid Data', 'Return quantity must be greater than 0', 'warning');
    return;
  }
  if (qty > returnRemainingQty.value) {
    await Swal.fire('Invalid Data', 'Return quantity exceeds the remaining balance', 'warning');
    return;
  }
...
async function submitClose() {
  ...
  try {
    ...
    await Swal.fire('Success', 'Special drug claim closed successfully', 'success');
...
<template>
  <div class="card">
    <div class="flex justify-between items-center mb-4">
      <div>
        ...
        <p class="text-surface-500 mt-1">Withdraw / Return records for special medicines</p>
      </div>
      <Button
        icon="pi pi-plus"
  ...
  >
    <div class="grid grid-cols-12 gap-3 mb-4">
      <div class="col-span-12">
        ...
        <Textarea v-model="createReason" rows="2" class="w-full" placeholder="Enter reason for special drug claim (Mandatory)" />
      </div>
        ...
        >
          ...
          <template #option="{ option }">
            <div class="flex justify-between w-full gap-2">
              <span class="truncate">{{ option.item_name_en || '-' }} [{{ option.item_name_th }}]</span>
      ...
      <div class="col-span-4">
        ...
        <div class="flex items-center gap-2">
          <InputNumber
            ...
            placeholder="Qty"
          />
          <div class="text-primary font-bold bg-primary/10 px-2 py-1 rounded border border-primary/20 whitespace-nowrap">
            {{ selectedStockItem ? (selectedStockItem.usage_unit_name_th || 'Unit') : 'Unit' }}
          </div>
        </div>
      </div>
    ...
    <div v-else-if="detail">
      <div class="grid grid-cols-12 gap-3 mb-4">
        <div class="col-span-4">
          ...
          <div>{{ formatSysdatetimeoffset(detail.header?.claim_datetime || '') }}</div>
        </div>
        <div class="col-span-4">
      ...
      <DataTable :value="detail.items" dataKey="claim_item_id" class="p-datatable-sm">
        <Column field="item_code" header="Code" />
        <Column header="Medicine / Item Name">
          <template #body="{ data }">
            {{ data.item_name_en || '-' }} [{{ data.item_name_th }}]
        ...
        <Column field="qty_issued_base" header="Total Issued" bodyClass="text-right">
          <template #body="{ data }">{{ formatNumber(data.qty_issued_base) }}</template>
        </Column>
        <Column header="Rem. Returnable" bodyClass="text-right">
          <template #body="{ data }">{{ formatNumber(getRemainingQty(data)) }}</template>
        </Column>
  ...
  >
    <div class="grid grid-cols-12 gap-3 mb-4">
      ...
      <div class="col-span-12">
        <label class="block mb-2 font-medium">Claim Reason</label>
        <Textarea v-model="editReason" rows="2" class="w-full" placeholder="Enter reason for adjustment (Mandatory)" />
      </div>
    </div>

    <DataTable :value="editItems" dataKey="claim_item_id" class="p-datatable-sm">
      <Column field="item_code" header="Code" />
      <Column header="Medicine / Item Name">
        <template #body="{ data }">
          {{ data.item_name_en || '-' }} [{{ data.item_name_th }}]
  ...
  >
    ...
    <div class="mb-3">
      ...
      <Textarea v-model="returnReason" rows="2" class="w-full" placeholder="Enter return reason (Mandatory)" />
    </div>

    <template #footer>
      <Button label="Cancel" severity="secondary" text @click="showReturnDialog = false" />
