  ...
  <div class="card">
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <Tag
          :value="
            '\u0E01\u0E33\u0E25\u0E31\u0E07\u0E43\u0E0A\u0E49: ' + (currentUserId || realUserId)
          "
          severity="info"
        />
      </div>
        ...
        <div class="flex justify-between items-center gap-2">
          ...
          <div class="flex gap-2">
            <Button
              ...
              rounded
              text
              @click="viewMode === 'pending' ? loadPendingApprovals() : loadHistoryApprovals()"
            />
            <IconField>
    ...
    <template v-if="selectedItem">
      ...
      <div v-if="selectedItem.type === 'PO' && poLines.length > 0" class="mb-4">
        ...
        <DataTable :value="poLines" size="small" stripedRows>
          ...
          >
            <template #body="{ data }">
              {{ data.item_name_th }}<br /><small class="text-surface-400">{{
                data.item_name_en
              }}</small>
            </template>
          </Column>
          ...
          >
            <template #body="{ data }"
              >&#x0E3F;{{
                Number(data.unit_price).toLocaleString('en-US', { minimumFractionDigits: 2 })
              }}</template
            >
          </Column>
          <Column :header="'\u0E23\u0E27\u0E21'" style="min-width: 100px">
            <template #body="{ data }"
              >&#x0E3F;{{
                Number(data.total_price).toLocaleString('en-US', { minimumFractionDigits: 2 })
              }}</template
            >
          </Column>
          <Column
          ...
          <template #content="{ item }">
            <div class="mb-3">
              <div class="flex items-center gap-2 mb-1">
                ...
                />
                <span v-if="item.approval_role !== 'SUBMITTER'" class="text-sm text-surface-500">{{
                  item.approval_role
                }}</span>
              </div>
              <div class="text-sm">
