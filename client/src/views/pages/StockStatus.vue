...
<template>
    <div class="card">
        ...
        <DataTable v-model:filters="filters" :value="numberedItems" :loading="loading" dataKey="item_id" paginator
            :rows="20" :rowsPerPageOptions="[10, 20, 50, 100]" stateStorage="local" stateKey="dt-stock-status"
            sortMode="multiple" removableSort scrollable scrollHeight="600px"
            :globalFilterFields="['item_code', 'item_name_en', 'item_name_th']" :rowClass="row => ({
            ...
            <Column header="สถานะ" style="min-width: 110px">
                <template #body="{ data }">
                    <Tag v-if="stockStatus(data) === 'low'" severity="danger" value="ต่ำกว่า MIN"
                        icon="pi pi-arrow-down" />
                    <Tag v-else-if="stockStatus(data) === 'high'" severity="warn" value="เกิน MAX"
                        icon="pi pi-arrow-up" />
