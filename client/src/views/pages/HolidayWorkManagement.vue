...
const messageTypeOptions = [
  { label: 'เข้าเวร 05:00 น. (ฝากกระเป๋ายาที่ป้อมยาม)', value: 'CHECK_IN_5AM' },
...
async function confirmDelete(data: IHolidayWorkAnnouncement) {
  ...
  if (result.isConfirmed) {
    try {
      ...
      await Swal.fire('Deleted!', 'Announcement has been deleted.', 'success');
      await loadAnnouncements();
    } catch (error: any) {
      await Swal.fire('Error', error.response?.data?.message || error.message, 'error');
    } finally {
      loading.value = false;
    }
  }
...
async function saveAnnouncement() {
  ...
  // Validation
  if (form.value.message_type === 'OTHER' && !form.value.custom_message?.trim()) {
    return;
  }
  ...
  try {
    ...
    const payload = {
      ...
      custom_message: form.value.message_type === 'OTHER' ? form.value.custom_message : null,
    };

    if (isEdit.value) {
      await HolidayWorkService.update(payload.work_date, payload);
      await Swal.fire('Updated!', 'Successfully updated the announcement.', 'success');
    } else {
      await HolidayWorkService.create(payload);
      await Swal.fire('Created!', 'Successfully created the announcement.', 'success');
    }
...
watch(() => form.value.message_type, (newType) => {
  if (newType !== 'OTHER') {
  ...
  <div class="card">
    <div class="flex justify-between items-center mb-4">
      <div>
        ...
        <p class="text-surface-500 mt-1">Manage announcements for holiday work shifts</p>
      </div>
      <Button
        icon="pi pi-plus"
    ...
    >
      ...
      <Column field="work_date" header="Date" sortable>
        <template #body="{ data }">
          <span class="font-bold text-primary">{{ formatDate(data.work_date) }}</span>
        </template>
      </Column>
      <Column header="Shift Counts" style="min-width: 150px">
        <template #body="{ data }">
          <div class="flex gap-2">
            ...
            <Tag severity="secondary" :value="`Night: ${data.night_shift_count}`" />
          </div>
        </template>
      </Column>
      <Column field="message_type" header="Message Type">
        <template #body="{ data }">
          <div class="flex flex-col">
            <span class="font-medium">{{ getMessageLabel(data.message_type) }}</span>
            <small v-if="data.message_type === 'OTHER'" class="text-surface-500 italic">
              "{{ data.custom_message }}"
            </small>
          </div>
        </template>
  ...
  >
    <div class="field mb-4">
      ...
      <DatePicker
        ...
        class="w-full"
      />
      <small v-if="isEdit" class="text-surface-500">Date cannot be changed once created.</small>
    </div>

    <div class="grid grid-cols-12 gap-4 mb-4">
      <div class="col-span-4">
        <label for="day_shift" class="block mb-2 font-medium">Day Shift Count</label>
        <InputNumber
          id="day_shift"
      ...
      <div class="col-span-4">
        <label for="night_shift" class="block mb-2 font-medium">Night Shift Count</label>
        <InputNumber
          id="night_shift"
    ...
    <div class="field mb-4">
      <label for="message_type" class="block mb-2 font-medium">Message Type</label>
      <Select
        id="message_type"
    ...
    <div v-if="form.message_type === 'OTHER'" class="field mb-4">
      <label for="custom_message" class="block mb-2 font-medium">Custom Message</label>
      <Textarea
        id="custom_message"
      ...
      />
      <small class="p-error" v-if="submitted && !form.custom_message">Custom message is required when type is 'OTHER'.</small>
    </div>
