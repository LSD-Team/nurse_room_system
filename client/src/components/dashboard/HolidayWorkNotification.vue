<script lang="ts" setup>
  ...
  const fetchAnnouncements = async (silent = false) => {
    try {
      ...
      if (
        JSON.stringify(newData) !== JSON.stringify(activeAnnouncements.value)
      ) {
    ...
    >
      <i
        :class="[
          'pi pi-bell',
          {
            'text-primary': activeAnnouncements.length > 0,
            'animate-swing': activeAnnouncements.length > 0 && !hasBeenViewed,
          },
        ]"
      ></i>
      ...
      <span
        ...
        class="absolute top-0 right-0 flex h-3 w-3 translate-x-1 -translate-y-1"
      >
        <span
          class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"
        ></span>
        <span
          class="relative inline-flex rounded-full h-3 w-3 bg-red-500 border-2 border-white"
        ></span>
      </span>
    </button>
    ...
    <Popover ref="op" style="width: 350px">
      <div class="p-1">
        <div class="flex items-center justify-between mb-3 pb-2 border-b">
          <span
            class="font-bold text-lg flex items-center gap-2 text-surface-900 dark:text-surface-0"
          >
          ...
          <Tag
            v-if="activeAnnouncements.length > 0"
            severity="danger"
            :value="`${activeAnnouncements.length} รายการ`"
          />
        </div>
        ...
        <div v-if="activeAnnouncements.length > 0" class="flex flex-col gap-4">
          ...
          >
            ...
            <div
              class="text-sm font-semibold text-surface-900 dark:text-surface-0 mb-2"
            >
              {{
                item.message_type === 'OTHER'
                  ? item.custom_message
            ...
            <div class="flex gap-2">
              <Tag
                severity="info"
                :value="`กลางวัน: ${item.day_shift_count}`"
                class="text-[10px]"
              />
              <Tag
                severity="secondary"
                :value="`กลางคืน: ${item.night_shift_count}`"
                class="text-[10px]"
              />
            </div>
          </div>
