...
export class HolidayWorkService {
  ...
  static async getActive(silent = false): Promise<IHolidayWorkAnnouncement[]> {
    return Api.get<IHolidayWorkAnnouncement[]>(`${this.BASE_URL}/active`, {}, {
      silent,
    } as any);
  }

  static async getAll(silent = false): Promise<IHolidayWorkAnnouncement[]> {
    return Api.get<IHolidayWorkAnnouncement[]>(`${this.BASE_URL}/all`, {}, {
      silent,
    } as any);
  }
  ...
  static async getByDate(
    date: string
  ): Promise<IHolidayWorkAnnouncement | null> {
    return Api.get<IHolidayWorkAnnouncement | null>(
      `${this.BASE_URL}/by-date`,
      {
        params: { date },
      }
    );
